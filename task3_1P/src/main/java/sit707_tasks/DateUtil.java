...
public class DateUtil {
	...
	public void increment() {
		if (day < monthDuration(month, year)) {
			...
			day++;
		} else if (month < 12) {			// Last day of a month AND month is less than December, so +1d is first day of next month.
			day = 1;
			month++;
		} else {			// Month is December, so +1d is 1st January next year.
			day = 1;
			month = 1;
			year++;
		}
